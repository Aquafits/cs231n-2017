...
class KNearestNeighbor(object):
    ...
    def compute_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                #####################################################################
                test_picture = X[i]
                training_picture = self.X_train[j]
                l2_distance = np.sqrt(np.sum(np.square(test_picture - training_picture)))
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            # label.                                                                #
            #########################################################################
            label_occurrences = np.bincount(closest_y)
            y_pred[i] = np.argmax(label_occurrences)
            #########################################################################
            #                           END OF YOUR CODE                            #
            #########################################################################
